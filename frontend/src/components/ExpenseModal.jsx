...
const ExpenseModal = ({ isOpen, onClose, expense, onSuccess }) => {
  ...
  // Predefined payment channels
  const paymentMethods = ['Card', 'Cash', 'Bank Transfer', 'Crypto Wallet', 'Other'];
  ...
  const handleSubmit = async (e) => {
    ...
    if (!title.trim()) return setError('Please enter a description or vendor title.');
    ...
    try {
      ...
      if (isEditMode) {
        console.log(`[Expense Modal] Updating expense ${expense._id}...`);
        await api.put(`/expenses/${expense._id}`, payload);
        showToast('Expense record successfully updated!', 'success');
      } else {
        console.log('[Expense Modal] Creating new expense log...');
        await api.post('/expenses', payload);
        showToast('New expense successfully logged!', 'success');
      }

      onSuccess(); // Refreshes the parent page table
      onClose();
    } catch (err) {
      console.error('[Expense Modal Error] Operation failed:', err.message);
      ...
      <div className="w-full max-w-md p-6 rounded-3xl glass-panel border border-primary/20 shadow-orchid-glow relative z-10 space-y-5 animate-scale-up">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-white tracking-tight">
              {isEditMode ? 'Modify Expense Log' : 'Record Outflow Expense'}
            </h2>
          </div>
          <button 
            onClick={onClose} 
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-left">
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Vendor / Title</label>
            <input 
              type="text"
            ...
            <div className="space-y-1.5">
              ...
              >
                {paymentMethods.map((pm) => (
                  <option key={pm} value={pm} className="bg-background text-white">{pm}</option>
                ))}
              </select>
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              placeholder="e.g. Card payment, dinner with team..."
              value={notes}
          ...
          <div className="pt-2">
            <button
              type="submit"
              disabled={loading}
              className={`w-full py-2.5 rounded-xl text-xs font-bold text-white bg-primary hover:bg-primary/95 transition-all transform hover:-translate-y-0.5 cursor-pointer flex items-center justify-center gap-1.5 shadow-orchid-glow
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  <span>{isEditMode ? 'Save Modifications' : 'Create Expense Log'}</span>
                </>
              )}
            </button>
          </div>
